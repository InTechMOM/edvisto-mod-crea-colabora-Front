...
let recognitionActiveEfectos = false;
let recognitionEfectos;
...
const toggleRecognitionEfectos = () => {
    const voiceButton = document.getElementById("voice-efectos-image");
     if (recognitionActiveEfectos) {
         recognitionEfectos.stop();
         voiceButton.setAttribute ('src', imageOffPath)
 
     } else {
         recognitionEfectos.start();
         voiceButton.setAttribute ('src', imageOnPath)
     }
     recognitionActiveEfectos = !recognitionActiveEfectos;
 };
...
// Si el navegador es compatible, configurar el reconocimiento de voz para efectos
if (elNavegadorEsCompatible()) {
    recognitionEfectos = new (window.SpeechRecognition || window.webkitSpeechRecognition || 
        window.mozSpeechRecognition || window.msSpeechRecognition)();
        recognitionEfectos.lang = "es-US";
        
        recognitionEfectos.onend = (event) => { 
            if (recognitionActiveEfectos) {
                recognitionEfectos.start(); 
            }
        };
        
        recognitionEfectos.onresult = (resultado) => { manejarResultadoEfectos(resultado); }; 
        
        document.getElementById("voice-efectos").addEventListener("click", toggleRecognitionEfectos);
    }
...
// Función que maneja el resultado del reconocimiento de voz para efectos
const manejarResultadoEfectos = resultado => {
    updateCounter("efectos", resultado.results[0][0].transcript);
};
